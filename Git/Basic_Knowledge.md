...
在工作区，如果某个branch存在untracked文件，这个时候如果切换到其他的branch，那么这些untracked文件也会一并保留，并且在新的branch下也是保持untracked状态。

#### Index / Staging Area
索引区，也是暂存区，可以使用`git add`把修改放到这里，告诉Git "这些修改我想要commit"。
`git add .` 将所有的改动放到暂存区。
`git add -u` 将所有tracked并且modified文件添加到暂存区。
...
`git commit --amend --no-edit` 将暂存区的内容添加到上一个提交，并不改变提交的message.
